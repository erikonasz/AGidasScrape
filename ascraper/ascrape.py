...
def get_cars(url, count_cars):
    ...
    while True:
        ...
        for car in allcars:
            ...
            if non_latin_char.search(title) or non_latin_char.search(price) or non_latin_char.search(
                    liter) or non_latin_char.search(fuel_type) or non_latin_char.search(year) or non_latin_char.search(
                    transmission) or non_latin_char.search(city):
                continue
...
def write_to_csv(cars, chunk_size=1000):
    with open("cars.csv", "w", newline='', encoding='utf-8-sig') as csv_file:
        ...
        for i in range(0, len(cars), chunk_size):
            ...
            for car in chunk:
                if not isinstance(car['title'], str) or not isinstance(car['price'], str):
                    continue
